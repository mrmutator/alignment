...
def generate_prepare_job(**params):
    ...
    with open(params['job_template_dir'] + "/template_zero_tree_prepare_job.txt", "r") as infile:
...
def generate_iteration_jobs(**params):
    ...
    with open(params['job_template_dir'] + "/template_zero_tree_worker_job.txt", "r") as infile:
        template = infile.read()
        job_file = template % params
    with open(params['job_dir'] + "/worker_job_it" + params["it_number"] + ".sh", "w") as outfile:
        outfile.write(job_file)

    with open(params['job_template_dir'] + "/template_zero_tree_update_job.txt", "r") as infile:
        template = infile.read()
        job_file = template % params
    ...
    if params["align1"]:
        with open(params['job_template_dir'] + "/template_zero_tree_evaluate_job.txt", "r") as infile:
            template = infile.read()
            job_file = template % params
